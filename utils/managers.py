...
class CI_Manager():
    ...
    def _get_state(self):
        ...
        forecast_indices = (np.arange(self.time_step + 1, self.time_step + 1 + self.future_steps) % len(self.norm_carbon))
        forecast = self.norm_carbon[forecast_indices]
...
class Weather_Manager():
    ...
    def reset(self, init_day=None, init_hour=None, seed=None):
        ...
        next_index = (self.time_step + 1) % len(self.temperature_data)
        self._next_temp = self.temperature_data[next_index]
        self._current_norm_temp = self.norm_temp_data[self.time_step]
        self._next_norm_temp = self.norm_temp_data[next_index]
...
class ElectricityPrice_Manager:
    ...
    def _load_data(self):
        ...
        candidate_paths = [
            f"data/electricity_prices/standardized/{self.location}/{self.simulation_year}/{self.location}_electricity_prices_{self.simulation_year}.csv",
            f"data/electricity_prices/processed/{self.location}/{self.simulation_year}/{self.location}_electricity_prices_{self.simulation_year}.csv",
        ]
        if self.location == "GB":
            candidate_paths.append(
                f"data/electricity_prices/processed/GB_incompleted/{self.simulation_year}/GB_electricity_prices_{self.simulation_year}.csv"
            )
        ...
        if filename is None:
            raise FileNotFoundError(
                "Electricity price file not found. Tried: "
                + ", ".join(os.path.join(parent_dir, path) for path in candidate_paths)
            )
        ...
        datetime_col = "Datetime (UTC)" if "Datetime (UTC)" in df.columns else "Datetime"
        price_col = next((col for col in df.columns if col.startswith("Price (") and col.endswith("/MWh)")), None)
        if price_col is None:
            raise ValueError(f"Expected MWh price column missing in {filename}. Columns: {list(df.columns)}")

        df[datetime_col] = pd.to_datetime(df[datetime_col])
        # df = df.sort_values("Datetime (UTC)")

        # Store as numpy array of prices
        self.original_prices = df[price_col].values
