...
class DistributeAverageCarbon(BaseRBCStrategy):
    """
    Assigns the task using annual-average carbon intensity instead of the
    current hourly value.

    This is the key baseline for testing whether annual averages mask the
    operational value of hour-resolved carbon-aware scheduling.
    而是看全年平均碳强度：哪个数据中心全年平均碳强度最低，并且资源足够运行这个任务
    """
    def __call__(self, task, datacenters: dict, logger: logging.Logger = None):
        if not datacenters:
            if logger: logger.error("AverageCarbon: No datacenters provided.")
            return None

        candidates = []
        for dc_name, dc in datacenters.items():
            if not (hasattr(dc, 'can_schedule') and dc.can_schedule(task)):
                continue
            if not hasattr(dc, 'ci_manager'):
                if logger: logger.warning(f"AverageCarbon: {dc_name} has no ci_manager.")
                continue
            ci_values = getattr(dc.ci_manager, "original_data", None)
            if ci_values is None:
                ci_values = getattr(dc.ci_manager, "carbon_smooth", None)
            if ci_values is None or len(ci_values) == 0:
                if logger: logger.warning(f"AverageCarbon: no CI data available for {dc_name}.")
                continue
            avg_ci = float(np.mean(ci_values))
            candidates.append((avg_ci, dc))

        if not candidates:
            if logger:
                logger.warning(f"AverageCarbon: Task {task.job_name} could not be assigned! No schedulable DC found with annual CI data.")
            return None

        candidates.sort(key=lambda item: item[0])
        best_avg_ci, best_dc = candidates[0]

        if logger:
            logger.info(f"AverageCarbon choice for task {task.job_name}: DC{best_dc.dc_id} (annual avg CI: {best_avg_ci:.2f} gCO2/kWh)")
        return best_dc.dc_id


class DistributeTimeShiftLowestCarbon(BaseRBCStrategy):
    """
    Defers flexible tasks until the current best carbon opportunity is good
    enough, then assigns them to the currently lowest-carbon schedulable DC.

    Returning 0 means "defer" and is handled by DatacenterClusterManager.
    它会先找当前最低碳强度的数据中心。如果当前最低碳强度还不够低，而且任务允许延迟：
 """
    def __init__(self, dispatch_threshold_ratio=0.85, urgent_ratio=0.85):
        self.dispatch_threshold_ratio = dispatch_threshold_ratio
        self.urgent_ratio = urgent_ratio
        super().__init__()

    def __call__(self, task, datacenters: dict, logger: logging.Logger = None):
        if not datacenters:
            if logger: logger.error("TimeShiftLowestCarbon: No datacenters provided.")
            return None

        candidates = []
        all_ci = []
        for dc_name, dc in datacenters.items():
            if not hasattr(dc, 'get_current_carbon_intensity'):
                continue
            try:
                ci = dc.get_current_carbon_intensity(norm=False)
                if ci is not None:
                    all_ci.append(float(ci))
                    if hasattr(dc, 'can_schedule') and dc.can_schedule(task):
                        candidates.append((float(ci), dc))
            except Exception as e:
                if logger: logger.error(f"TimeShiftLowestCarbon: Error getting CI for {dc_name}: {e}")

        if not candidates:
            if logger:
                logger.warning(f"TimeShiftLowestCarbon: No schedulable DC for task {task.job_name}.")
            return None

        candidates.sort(key=lambda item: item[0])
        best_ci, best_dc = candidates[0]
        mean_ci = float(np.mean(all_ci)) if all_ci else best_ci

        elapsed_minutes = 0.0
        if getattr(task, "arrival_time", None) is not None and getattr(best_dc, "current_time_task", None) is not None:
            try:
                elapsed_minutes = max(0.0, (best_dc.current_time_task - task.arrival_time).total_seconds() / 60.0)
            except Exception:
                elapsed_minutes = 0.0

        max_delay = getattr(task, "max_delay_minutes", 0.0)
        near_deadline = max_delay <= 0 or elapsed_minutes >= self.urgent_ratio * max_delay
        can_defer = bool(getattr(task, "defer_allowed", False)) and not near_deadline
        carbon_window_is_good = best_ci <= mean_ci * self.dispatch_threshold_ratio

        if can_defer and not carbon_window_is_good:
            if logger:
                logger.info(
                    f"TimeShiftLowestCarbon defers task {task.job_name}: "
                    f"best CI={best_ci:.2f}, mean CI={mean_ci:.2f}, threshold={mean_ci * self.dispatch_threshold_ratio:.2f}"
                )
            return 0

        if logger:
            logger.info(f"TimeShiftLowestCarbon choice for task {task.job_name}: DC{best_dc.dc_id} (CI: {best_ci:.2f} gCO2/kWh)")
        return best_dc.dc_id
...
class DistributeLocalOnly(BaseRBCStrategy): #任务来源是哪个 DC，就放回哪个 DC
