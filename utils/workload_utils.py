...
def classify_ai_task(duration, cores_req, gpu_req, bandwidth_gb):
    """
    Assign a coarse AI workload class from trace-derived resource features.

    The thresholds are intentionally simple so experiments remain transparent:
    long or GPU-heavy jobs behave like shiftable training, short jobs behave like
    latency-sensitive inference, and the rest are batch jobs.
    """
    if gpu_req >= 1.0 or duration >= 120:
        return "training"
    if duration <= 15 and bandwidth_gb <= 2.0 and gpu_req < 0.25:
        return "inference"
    return "batch"
...
def extract_tasks_from_row(row, scale=1, datacenter_configs=None,
    ...
    for task_data in row['tasks_matrix']:
        ...
        task_type = classify_ai_task(duration, cores_req, gpu_req, bandwidth_gb)
        ...
        for i in range(scale - 1):
            ...
            new_task = Task(
                ...
                task_type=classify_ai_task(duration, varied_cpu, varied_gpu, varied_bw),
            )
    ...
    if group_size == 1:
        ...
        for i in range(num_groups):
            ...
            meta_task = Task(
                ...
                task_type=classify_ai_task(agg_duration, agg_cores_req, agg_gpu_req, agg_bandwidth_gb),
            )
